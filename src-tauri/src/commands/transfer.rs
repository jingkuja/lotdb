...
// ─── Value extractors (reused from data.rs pattern) ──────────────
...
fn write_sql_insert(
    ...
    for row in rows {
        ...
        writeln!(writer, "INSERT INTO {table_ref} ({col_list}) VALUES ({vals});")?;
    }
    Ok(())
}
...
fn write_excel(
    ...
    for (col_idx, col_name) in columns.iter().enumerate() {
        let width = (col_name.len() as f64 + 2.0).max(12.0).min(40.0);
        worksheet
...
pub async fn export_table_data(
    ...
    if format == "excel" {
        let (col_names, rows) = fetch_all_as_json(&entry.pool, &database, schema.as_deref(), &table, &columns, where_clause.as_deref(), limit).await?;
        write_excel(&file_path, &col_names, &rows)?;
        return Ok(ExportResult { rows_exported: rows.len() as u64, file_path });
    }

    // Build SQL and open output file
    let (table_ref, col_select, db_quote): (String, String, fn(&str) -> String) =
        match &entry.pool {
            DbPool::MySQL(_) => {
            ...
            match &entry.pool {
                DbPool::MySQL(pool) => {
                    ...
                    while let Some(row) = stream.next().await {
                        ...
                        let vals: Vec<serde_json::Value> = (0..row.columns().len()).map(|i| mysql_val(&row, i)).collect();
                        let obj: serde_json::Map<String, serde_json::Value> = col_names.iter().cloned().zip(vals).collect();
                        if rows_exported > 0 { write!(writer, ",").map_err(|e| format!("写入失败: {e}"))?; }
                        let s = serde_json::to_string(&serde_json::Value::Object(obj)).map_err(|e| format!("序列化失败: {e}"))?;
                ...
                DbPool::Postgres(pool) => {
                    ...
                    while let Some(row) = stream.next().await {
                        ...
                        let vals: Vec<serde_json::Value> = (0..row.columns().len()).map(|i| pg_val(&row, i)).collect();
                        let obj: serde_json::Map<String, serde_json::Value> = col_names.iter().cloned().zip(vals).collect();
                        if rows_exported > 0 { write!(writer, ",").map_err(|e| format!("写入失败: {e}"))?; }
                        let s = serde_json::to_string(&serde_json::Value::Object(obj)).map_err(|e| format!("序列化失败: {e}"))?;
                        write!(writer, "\n  {s}").map_err(|e| format!("写入失败: {e}"))?;
            ...
            match &entry.pool {
                DbPool::MySQL(pool) => {
                    ...
                    while let Some(row) = stream.next().await {
                        ...
                        let vals: Vec<serde_json::Value> = (0..row.columns().len()).map(|i| mysql_val(&row, i)).collect();
                        let col_list = col_names.iter().map(|c| db_quote(c)).collect::<Vec<_>>().join(", ");
                        let val_list = vals.iter().map(sql_val).collect::<Vec<_>>().join(", ");
                        writeln!(writer, "INSERT INTO {table_ref} ({col_list}) VALUES ({val_list});")
                            .map_err(|e| format!("写入失败: {e}"))?;
                        rows_exported += 1;
                        emit_export_progress!();
                    }
            ...
            match &entry.pool {
                DbPool::MySQL(pool) => {
                    ...
                    while let Some(row) = stream.next().await {
                        ...
                        if col_names.is_empty() {
                            col_names = row.columns().iter().map(|c| c.name().to_string()).collect();
                            writeln!(writer, "{}", col_names.join(",")).map_err(|e| format!("写入失败: {e}"))?;
                        }
                        let vals: Vec<String> = (0..row.columns().len()).map(|i| csv_cell(&mysql_val(&row, i))).collect();
                        writeln!(writer, "{}", vals.join(",")).map_err(|e| format!("写入失败: {e}"))?;
                        rows_exported += 1;
                        emit_export_progress!();
                    }
                }
                DbPool::Postgres(pool) => {
                    ...
                    while let Some(row) = stream.next().await {
                        ...
                        let vals: Vec<String> = (0..row.columns().len()).map(|i| csv_cell(&pg_val(&row, i))).collect();
                        writeln!(writer, "{}", vals.join(",")).map_err(|e| format!("写入失败: {e}"))?;
                        rows_exported += 1;
                        emit_export_progress!();
                    }
    ...
    Ok(ExportResult { rows_exported, file_path })
}
...
async fn fetch_all_as_json(
    ...
    let (q_table, col_select) = match pool {
        DbPool::MySQL(_) => {
            let tr = format!("`{database}`.`{table}`");
            let cs = if columns.is_empty() { "*".to_string() }
                else { columns.iter().map(|c| format!("`{c}`")).collect::<Vec<_>>().join(", ") };
            (tr, cs)
        }
        DbPool::Postgres(_) => {
    ...
    match pool {
        DbPool::MySQL(p) => {
            let db_rows = sqlx::query(&sql).fetch_all(p).await.map_err(|e| format!("查询失败: {e}"))?;
            let col_names = db_rows.first().map(|r| r.columns().iter().map(|c| c.name().to_string()).collect()).unwrap_or_default();
            let rows = db_rows.iter().map(|r| (0..r.columns().len()).map(|i| mysql_val(r, i)).collect()).collect();
            Ok((col_names, rows))
        }
        DbPool::Postgres(p) => {
            let db_rows = sqlx::query(&sql).fetch_all(p).await.map_err(|e| format!("查询失败: {e}"))?;
            let col_names = db_rows.first().map(|r| r.columns().iter().map(|c| c.name().to_string()).collect()).unwrap_or_default();
            let rows = db_rows.iter().map(|r| (0..r.columns().len()).map(|i| pg_val(r, i)).collect()).collect();
            Ok((col_names, rows))
        }
    }
}
...
pub async fn batch_export_tables(
    ...
    for (table_index, table) in tables.iter().enumerate() {
        app.emit("export-batch-progress", BatchExportProgress {
            table_index,
            total_tables,
            table: table.clone(),
        }).ok();
        ...
        let write_result = if format == "excel" {
            ...
            (|| -> Result<(), String> {
                ...
                match format.as_str() {
                    ...
                    "sql_insert" => {
                        let (table_ref, db_quote): (String, fn(&str) -> String) =
                            match &entry.pool {
                                DbPool::MySQL(_) => (
...
pub async fn import_table_data(
    ...
    if format == "sql" {
        ...
        for (idx, stmt) in stmts.iter().enumerate() {
            let res = match &entry.pool {
                DbPool::MySQL(p) => sqlx::query(stmt).execute(p).await.map(|r| r.rows_affected()),
                DbPool::Postgres(p) => {
                    sqlx::query(stmt).execute(p).await.map(|r| r.rows_affected())
                }
            };
            match res {
            ...
            if current % 50 == 0 || current == total {
                ...
                let eta = if rps > 0.0 { (total - current) as f64 / rps } else { 0.0 };
    ...
    if format == "json" {
        ...
        if col_names.is_empty() || str_rows.is_empty() {
            return Ok(ImportResult { rows_imported: 0, rows_failed: 0, error_message: None });
        }
        if truncate_first {
            run_truncate(&entry.pool, &database, schema.as_deref(), &table).await?;
        }
        ...
        let (table_ref, col_quote): (String, fn(&str) -> String) = match &entry.pool {
            ...
            DbPool::Postgres(_) => (format!("\"{schema_name}\".\"{table}\""), |c| format!("\"{c}\"")),
        };
        let col_list = mapped_indices.iter().map(|(_, t)| col_quote(t)).collect::<Vec<_>>().join(", ");
        ...
        for (idx, row) in str_rows.iter().enumerate() {
            let val_list = mapped_indices.iter()
                .map(|(src_idx, _)| {
                    let v = row.get(*src_idx).and_then(|o| o.as_deref());
                    match v {
                        None => "NULL".to_string(),
                        Some(s) => match &entry.pool {
                            DbPool::MySQL(_) => { let esc = s.replace('\\', "\\\\").replace('\'', "\\'"); format!("'{esc}'") }
                            DbPool::Postgres(_) => { let esc = s.replace('\'', "''"); format!("'{esc}'") }
                        },
                    }
                })
                .collect::<Vec<_>>().join(", ");
            ...
            let res = match &entry.pool {
                DbPool::MySQL(p) => sqlx::query(&insert_sql).execute(p).await.map(|r| r.rows_affected()),
                DbPool::Postgres(p) => sqlx::query(&insert_sql).execute(p).await.map(|r| r.rows_affected()),
            };
            match res {
                Ok(n) => rows_imported += n,
                Err(e) => { rows_failed += 1; if first_error.is_none() { first_error = Some(e.to_string()); } }
            }
            let current = (idx + 1) as u64;
            if current % 100 == 0 || current == total {
                let elapsed = start.elapsed().as_secs_f64().max(0.001);
                let rps = current as f64 / elapsed;
                let eta = if rps > 0.0 && current < total { (total - current) as f64 / rps } else { 0.0 };
                app.emit("import-progress", ImportProgress { current, total, rows_per_sec: rps, eta_sec: eta }).ok();
            }
        }
        return Ok(ImportResult {
    ...
    let col_names: Vec<String> = if has_header {
        ...
        match first {
            None => return Ok(ImportResult { rows_imported: 0, rows_failed: 0, error_message: None }),
    ...
    for (idx, record) in rdr2.records().enumerate() {
        ...
        if current % 100 == 0 || current == total_rows {
            let elapsed = start.elapsed().as_secs_f64().max(0.001);
            let rps = current as f64 / elapsed;
            let eta = if rps > 0.0 && current < total_rows { (total_rows - current) as f64 / rps } else { 0.0 };
            app.emit("import-progress", ImportProgress { current, total: total_rows, rows_per_sec: rps, eta_sec: eta }).ok();
        }
    }
...
pub async fn transfer_table_data(
    ...
    if src_rows.is_empty() {
        return Ok(TransferResult { rows_transferred: 0, rows_failed: 0, error_message: None });
    }
    ...
    if truncate_first {
        run_truncate(&tgt_entry.pool, &tgt_database, tgt_schema.as_deref(), &tgt_table).await?;
    }

    // ── 4. Build target table ref + quoting ───────────────────────
    let (tgt_table_ref, tgt_col_quote): (String, fn(&str) -> String) = match &tgt_entry.pool {
        DbPool::MySQL(_) => (
            format!("`{tgt_database}`.`{tgt_table}`"),
            |c| format!("`{c}`"),
        ),
        DbPool::Postgres(_) => {
