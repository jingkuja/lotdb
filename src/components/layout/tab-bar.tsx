    ...
    >
      <span
        ...
        style={{
          backgroundColor:
            conn?.color ||
            (conn?.dbType === "postgres" ? "#0284c7" : "#2563eb"),
        }}
      />
      ...
      <span className="max-w-32 truncate text-[10px] text-muted-foreground">
        {conn?.name}
        {tab.metadata?.database ? ` / ${String(tab.metadata.database)}` : ""}
      </span>
      {conn?.readonly && <Lock className="size-3" aria-label="只读" />}
      {dirty && (
        <span className="text-amber-500" aria-label="有未提交变更">
          ●
        </span>
      )}
      <button
...
export function TabBar() {
  ...
  return (
    <div
      role="tablist"
      aria-label="工作区标签"
      className="flex h-8 shrink-0 items-center overflow-x-auto border-b border-border bg-muted/30"
    >
      {tabs.map((tab) => (
